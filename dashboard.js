// dashboard.js - CORREÇÃO: REORGANIZAR FUNÇÕES
// ============================================
...
function updateUserUI() {
    const user = AppState.userProfile || window.authManager?.userProfile;
    
    const welcomeTitle = document.getElementById('welcomeTitle');
    if (welcomeTitle && user?.name) {
        welcomeTitle.textContent = `Bem-vindo(a), ${user.name}!`;
    }
...
function createPostElement(post) {
    ...
    const hasLiked = AppState.currentUser ? likedBy.includes(AppState.currentUser.uid) : false;
    ...
    postDiv.innerHTML = `
        ...
        ${post.hasMedia && post.mediaBase64 ? `
...
async function toggleLike(postId, currentLikes) {
    try {
        if (!AppState.currentUser) {
            showToast('error', 'Você precisa estar logado para curtir');
            return;
        }
...
async function loadDashboardStats() {
    try {
        ...
        const postsTodayElement = document.getElementById('postsTodayCount');
        const totalPostsElement = document.getElementById('totalPostsCount');
        
        if (postsTodayElement) postsTodayElement.textContent = postsToday;
        if (totalPostsElement) totalPostsElement.textContent = postsToday;
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        // Inicializar tema primeiro (função já declarada)
        initTheme();
